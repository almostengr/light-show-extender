...
public sealed class FppStartSequenceService : IFppStartSequenceService
{
    ...
    public async Task<Result<SequenceSelectorResource>> ExecuteAsync(SequenceSelectorResource resource)
    {
        try
        {
            ...
            IList<SequenceRule> rules = GetSequenceRules(resource.CurrentDate.Year);
